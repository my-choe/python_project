...
# 적 enemy 캐릭터
enemy = pygame.image.load("D:/PythonWorkspace/pygame_basic/enemy.png")
enemy_size = enemy.get_rect().size #캐릭터 이미지 크기
...
#폰트 정의
game_font = pygame.font.Font(None, 40) #폰트객체생성(폰트, 크기) none일 경우 기본폰트
...
while running:
    ...
    character_rect.left = character_x_pos # 현재 위치로 변경
    character_rect.top = character_y_pos # 현재 위치로 변경

    emeny_rect = enemy.get_rect()
    emeny_rect.left = enemy_x_pos
    emeny_rect.top = enemy_y_pos

    #충돌 체크
    if character_rect.colliderect(emeny_rect): #사각형 기준으로 충돌이 있었는지 확인
        print("충돌!")
        running = False
    ...
    # 타이머 집어넣기
    #경과 시간 계산
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000 #경과시간(ms) 초(s) 단위로 표시 
    timer = game_font.render(str(int(total_time - elapsed_time)), True, (255,255,255)) #render(출력할 글자, true, (튜플 RGB형태 글자색상))
